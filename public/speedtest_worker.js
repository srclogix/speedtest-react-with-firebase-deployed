...
var settings = {
	...
	url_dl: "https://x62on2liuf.execute-api.us-east-1.amazonaws.com/dev/garbage", // path to a large file or garbage.php, used for download test. must be relative to this js file
	url_ul: "https://x62on2liuf.execute-api.us-east-1.amazonaws.com/dev/empty", // path to an empty file, used for upload test. must be relative to this js file
	url_ping: "https://x62on2liuf.execute-api.us-east-1.amazonaws.com/dev/empty", // path to an empty file, used for ping test. must be relative to this js file
	url_getIp: "https://x62on2liuf.execute-api.us-east-1.amazonaws.com/dev/getIP", // path to getIP.php relative to this js file, or a similar thing that outputs the client's ip
	...
	garbagePhp_chunkSize: 1, // size of chunks sent by garbage.php (can be different if enable_quirks is active)
	...
	var testStream = function(i, delay) {
		setTimeout(
			function() {
				...
				xhr[i].open("POST", settings.url_dl + url_sep(settings.url_dl) + (settings.mpot ? "cors=true&" : "") + "r=" + Math.random() + "&ckSize=" + settings.garbagePhp_chunkSize, true); // random string to prevent caching
				xhr[i].send();
			}.bind(this),
			1 + delay
		);
